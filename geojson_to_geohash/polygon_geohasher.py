...
def geohash_to_polygon(geo):
    ...
    """
    lat_centroid, lng_centroid, lat_offset, lng_offset = geohash_1.decode_exactly(geo)
...
def polygon_to_geohashes(polygon, precision, inner=True):
    ...
    testing_geohashes.put(geohash_1.encode(centroid.y, centroid.x, precision))
            ...
            if condition:
                if inner:
                    ...
                    if polygon.intersects(current_polygon):
                        ...
                        outer_geohashes.add(current_geohash)
                for neighbor in geohash_1.neighbors(current_geohash):
